...
/* ===== Padronização global de competências das fontes · v2 ===== */
(() => {
  "use strict";
  ...
  function ansCompetence(scope) {
    return formatCompetence(document.getElementById(`${scope}-ans-month`)?.textContent);
  }

  async function leitosCompetence() {
    const cod6 = cityId().slice(0, 6);
    if (!cod6) return null;
    if (lastCityKey === cod6 && cachedLeitosComp) return cachedLeitosComp;

    try {
      const response = await fetch(`/proxy/elastic/leitos?cod=${encodeURIComponent(cod6)}`);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      cachedLeitosComp = formatCompetence(data?.comp);
      lastCityKey = cod6;
      return cachedLeitosComp;
    } catch (error) {
      console.warn("Falha ao consultar competência ElastiCNES:", error);
      return null;
    }
  }
  ...
  function cardSource(id) {
    return document.getElementById(id)?.closest(".card")?.querySelector("p.muted");
  }

  function updateElastiSources(comp) {
    if (!comp) return;

    setText(cardSource("micro-leitos-priv-evol"), `Fonte: CNES · ElastiCNES · snapshots anuais; última competência: ${comp}.`);
    setText(cardSource("city-leitos-priv-evol"), `Fonte: CNES · ElastiCNES · snapshots anuais; última competência: ${comp}.`);
    setText(cardSource("micro-leitos-table"), `Fonte: CNES · ElastiCNES · competência ${comp}. Comparativo: microrregião de Porto Alegre.`);
    setText(cardSource("city-leitos-table"), `Fonte: CNES · ElastiCNES · competência ${comp}.`);

    const top3Card = document.getElementById("top3-hospitals")?.closest(".card");
    if (top3Card) {
      setText(
        top3Card.querySelector("p.muted"),
        `Ranking por número total de leitos. Fonte: CNES · ElastiCNES · competência ${comp}.`
      );
    }

    document.querySelectorAll("p.muted").forEach(node => {
      const value = String(node.textContent || "");
      if (/CNES\s*·?\s*ElastiCNES/i.test(value) && /snapshot anual|snapshots anuais|última competência disponível/i.test(value)) {
        setText(node, `Fonte: CNES · ElastiCNES · snapshots anuais; última competência: ${comp}.`);
      }
    });
  }

  function updateAnsSources() {
    const city = ansCompetence("city");
    const micro = ansCompetence("micro") || city;

    if (micro) {
      setText(cardSource("micro-ans-evol"), `Fonte: ANS · PDA 024 · competência publicada: ${micro}.`);
    }

    if (city) {
      const card = document.getElementById("city-ans-evol")?.closest(".card");
      if (card) {
        let source = card.querySelector("p.muted[data-source-competence='ans-global']");
        if (!source) {
          source = document.createElement("p");
          source.className = "muted";
          source.dataset.sourceCompetence = "ans-global";
          card.appendChild(source);
        }
        setText(source, `Fonte: ANS · PDA 024 · competência publicada: ${city}.`);
      }
    }
  }

  function normalizeEmbeddedCompetences() {
    ["city-med-density-comp", "city-ans-month", "micro-ans-month"].forEach(id => {
      const node = document.getElementById(id);
      if (!node) return;
      const formatted = formatCompetence(node.textContent);
      if (formatted && node.textContent !== formatted) node.textContent = formatted;
    });
  }

  async function refresh() {
    normalizeEmbeddedCompetences();
    updateAnsSources();
    updateElastiSources(await leitosCompetence());
  }
